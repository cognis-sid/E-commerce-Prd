...
public class PRE_GET_DSTNTN_CNTRY_COMBO extends AncestorJdbcDao{

	JdbcTemplate jdbcTemplate;
	Map map;
	String query1;
	public PRE_GET_DSTNTN_CNTRY_COMBO(JdbcTemplate jdbcTemplate,Map map) {
	...
	public void executeDbProcedure() {
		...
		if ("Y".equalsIgnoreCase((String) map.get("P_I_V_REP_FLAG"))
			...
			query1 = "SELECT   " + "              I30.SCCODE as \"CODE\" ," + "              I30.SCNAME as \"DESC\"   "
					...
					+ "            WHERE   " + "              I60.VVVERS = 99   "
					+ "              AND I60.VVLDDS IN ('L', 'D', 'B')   " + "              AND I30.SCRCST = 'A'  and I30.SCCODE = '"+map.get("P_I_V_PORT_OF_DISCHARGE").toString().substring(0,2)+"'  "
					+ "              AND   " + "                I30.SCCODE IN (   " + "                SELECT   "
					+ "                  I40.PICNCD   " + "                FROM   " + "                  ITP040 I40   "
					+ "                WHERE   " + "                  I40.PICODE IN (   "
					+ "                    SELECT DISTINCT   " + "                      DTL.POD   "
					+ "                    FROM   "
					+ "                      ESV_CONTRACT_PARTY_PP_HDR HDR LEFT JOIN ESV_CONTRACT_PARTY_PP_DTL DTL ON DTL.FK_ESV_PP_HDR_SEQ = HDR.PK_ESV_PP_HDR_SEQ   "
					+ "                    WHERE   " + "                      HDR.FK_CONTRACT_PARTY_CODE = '"
					+ map.get("P_I_V_REP_CODE") + "'" + "                      AND (   "
			...
			query1=" select pk_country_code AS  \"code\" ,country_name as  \"DESC\"  from cam_country where pk_country_code=(select fk_country_code from cam_point cp where pk_point_code='"+map.get("P_I_V_PORT_OF_DISCHARGE")+"') limit 1";
		}
		 

		//String 	query1="select country as \"code\" ,name as \"DESC\" from VRL_PORT where code='"+map.get("P_I_V_PORT_OF_DISCHARGE")+"' ";
		System.out.println("PRE_GET_DSTNTN_CNTRY_COMBO : " +query1);
